...
我们让外层div `container` 的左右内间距分别等同于左右边栏的宽度:`padding: 0 200px;`,这个时候，依然是竖着一列，别急，我们先让他们处于同一行位置，怎么做呢，利用 **浮动情况下，负边界值会导致div上移**,所以将这个三个区域都增加`float:left;`,left区域`margin-left: -100%;`会叠加到main区域上方，right区域`margin-left: -200px`,也会叠加到main区上方。
...
这个时候，和圣杯布局一样的毛病，竖着排三行。接下来，其实和圣杯的想法一样，利用 **浮动情况下，负边界值会导致div上移**,所以将这个三个区域都增加`float:left;`,并给 `wrap` 清除浮动。
...
不过，看到我们的展示图，中间区域的内容区，被左右区域给覆盖住了，那这个好办，给`main-content`这个区域来 `padding: 0 200px;`,就是左右区域的宽度，最终效果
...
- 双飞翼则将 `padding`放在了`main`内的 div上，仅用了 `float`布局、`margin` 负边距


貌似，看起来双飞翼更简单，那么这套布局的优点有：
